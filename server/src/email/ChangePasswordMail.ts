...
const ChangePasswordMail = async (userId: number, otp: number) => {
  const user = await getUserInstance(userId);
  const displayName = user.getDisplayName();

  return ` <p style="font-size:36px;line-height:42px;margin:30px 0;color:#1d1c1d;font-weight:700;padding:0">Change Password</p>
  <p style="font-size:20px;line-height:28px;margin:16px 0;margin-bottom:30px">Hey ${displayName}, You request to change your password. Your verification code is:</p>
